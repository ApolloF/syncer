...
func TestPrunePinnedPointsStay(t *testing.T) {
	...
	if !got[at["A"]] {
		t.Error("pinned point A (40 days) expired with the usual history")
	}
	if got[old] || got[at["E"]] {
		t.Error("pin kept a point over a year old, or E wasn't thinned")
	}
	if pins(filepath.Join(target, VersionsDir, id))[old.Format(stampFmt)] {
		t.Error("stale pin for the expired point left behind")
	}
}
