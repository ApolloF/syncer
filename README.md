...
- **Works with game launchers.** A local API ([docs/api.md](docs/api.md)) lets a launcher such as [WaterLauncher](https://github.com/ApolloF/WaterLauncher) show a game's save status, sync its saves before it starts and back them up after it exits. When the window isn't open, the launcher starts `Syncer.exe --api`, which stops again once it's idle.
