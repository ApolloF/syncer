...
- **Knows what Steam Cloud really covers.** A game supporting Steam Cloud isn't enough. Syncer leaves a save to Steam only when every check passes: Steam installed the game (it has an app manifest); the game folder has no crack or Steam emulator files (`steam_emu.ini`, Goldberg's `steam_settings`, or a `steam_api` DLL without a valid Valve signature); the Steam account signed in on this PC keeps cloud saves for it; Steam Cloud tracks files in that very folder; the folder holds no mod saves Steam skips (SKSE/F4SE co-saves, Seamless Co-op `.co2`); and Steam has seen the latest save (a save changed later means the game was played without Steam). If any check fails, Syncer syncs the game like any other and the *Found on this PC* list says why (*Not installed through Steam*, *Cracked copy (RUNE)*, *Played outside Steam*, …). You can choose to include confirmed Steam Cloud games too.
...
- **Back up without syncing.** Turn off *Sync* for a game (or pick *Back up only* when adding one) and it keeps being backed up from this PC without being shared with your other PCs. These games are under *Games → Backup*, with when each was last backed up, its size and its restore points. Turn off both *Sync* and *Backup* and the game stays in the list as *Off*: Syncer leaves it alone (and never adds it back) until you turn one on again.
- **Only installed games.** With *Settings → Only sync installed games* on, games from your other PCs and new games found here are added only when they're installed here (Steam, Epic, GOG, Ubisoft, Xbox and regular installers are recognized). *Stop syncing them* on the Games page stops syncing the ones that aren't installed; they start again once you install them. *Games → On other PCs* lists everything else so you can add it by hand.
...
- **Brings back old backups.** *Games → Backup → Other backups in Google Drive* lists backups no game on this PC uses: games you removed here, or ones another PC backs up to the same account. *Add to Syncer* backs one up from this PC again (its history is kept) and can restore its saves right away.
